"""
RSI Hidden Divergence Strategy - VERSION 3 (Stability-Hardened)

Changes from v1/v2 targeting the 5 fragile parameters:

1. rsi_period (was 0% stable): REMOVED as optimizable param.
   Now uses multi-period RSI consensus (avg of 7, 14, 21).
   The consensus smooths over any single period's fragility.

2. swing_strength (was 0% stable): REMOVED as optimizable param.
   Now uses adaptive swing detection - tries multiple strengths
   and takes the consensus. No single strength value can break it.

3. trend_ma_period (was 0.001% stable): REMOVED as optimizable param.
   Now uses dual-MA trend filter (50 + 200 SMA agreement).
   No single MA period cliff edge.

4. sl_atr_mult (was 0% stable): Replaced with ATR-band approach.
   SL is now set at a percentage of recent ATR range, making it
   continuously adaptive rather than a cliff-edge multiplier.

5. be_trigger_pips (was 21% stable): Now ATR-scaled.
   be_trigger = be_atr_mult * ATR, adapts to volatility.

Net result: 30 params (5 fragile params removed/replaced).
Signal quality should be more robust across parameter perturbations.

Created: 2026-02-06
Based on stability analysis of: GBP_USD_H1_20260206_011323 (93/100 GREEN)
Goal: Improve stability scores while maintaining or improving overall rating.
"""
import numpy as np
import pandas as pd
...
class RSIDivergenceFullFastV3(FastStrategy):
    """
    RSI Hidden Divergence - VERSION 3 (Stability-Hardened).
    ...
    1. signal (6 params): RSI divergence settings (no rsi_period, no swing_strength)
    ...
    Total: 30 parameters (down from 35, with fragile ones eliminated)
    """

    name = "RSI_Divergence_v3"
    version = "3.0"
    ...
    def get_parameter_groups(self) -> Optional[Dict[str, ParameterGroup]]:
        ...
        # === Group 1: Signal Generation (6 params) ===
        # NOTE: rsi_period and swing_strength REMOVED - now hardcoded as consensus
        ...
        # === Group 2: Entry Filters (5 params) ===
        # NOTE: trend_ma_period REMOVED - now hardcoded as dual-MA (50+200)
        # Default use_trend_filter=False so signal stage can find trades without restrictive filter
        ...
        # === Group 3: Risk (SL/TP) (5 params) ===
        # NOTE: sl_atr_mult REMOVED - replaced with sl_atr_pct (percentage-based)
        # Default sl_atr_pct=150 (1.5x ATR) - permissive default for signal stage
        ...
        risk.add_param('tp_atr_mult', [2.0, 3.0, 4.0, 6.0], default=3.0)
        risk.add_param('tp_fixed_pips', [30, 50, 75, 100], default=50)
        groups['risk'] = risk

        # === Group 4: Trade Management (8 params) ===
        # NOTE: be_trigger_pips replaced with be_atr_mult (ATR-scaled)
        # Defaults are OFF so signal stage tests pure signal quality
        ...
        management.add_param('be_offset_pips', [0, 2, 5], default=0)
    ...
    def get_management_arrays(
        ...
        be_offset = np.full(n, params.get('be_offset_pips', 0), dtype=np.float64)
        ...
        # Partial target based on TP distance
    ...
    def _get_sl_pips(self, signal: FastSignal, params: Dict[str, Any]) -> float:
        ...
        if sl_mode == 'fixed':
            ...
            # V3: Percentage-based ATR SL (smoother than multiplier)
    ...
    def precompute(self, df: pd.DataFrame) -> List[FastSignal]:
        """
        V3: Pre-compute signals using multi-period RSI consensus
        and adaptive swing detection.

        Instead of generating signals for EACH RSI period/swing strength
        and filtering later, we:
        1. Compute RSI for all 3 consensus periods
        2. Average them for a consensus RSI
        3. Find swings across multiple strengths (union)
        4. Generate signals from the consensus

        This eliminates rsi_period and swing_strength as optimizable params.
        """
        signals = []
        ...
        # V3: Adaptive swing detection - union of multiple strengths
        # Find swings from all strengths and merge
        all_swing_lows = {}  # bar -> (bar, price)
        all_swing_highs = {}  # bar -> (bar, price)
        ...
        # The maximum swing strength used determines the confirmation delay
    ...
    def filter_signals(
        ...
        """
        V3: Filter signals. No rsi_period or swing_strength filtering needed
        since we use consensus.
        """
        result = []
        ...
        for s in signals:
            ...
            # === V3: Dual-MA Trend Filter ===
            # Both MA50 and MA200 must agree on trend direction
            if params.get('use_trend_filter', True):
                ma50 = attr.get('ma50', 0)
                ma200 = attr.get('ma200', 0)
                if ma50 > 0 and ma200 > 0:
                    if s.direction == 1:
                        # Buy: price must be above BOTH MAs
                        if s.price < ma50 or s.price < ma200:
                            continue
                    else:
                        # Sell: price must be below BOTH MAs
                        if s.price > ma50 or s.price > ma200:
                            continue
    ...
    def _filter_vectorized(self, params: Dict[str, Any]) -> np.ndarray:
        ...
        if params.get('require_pullback', False):
            rsi_overbought = params.get('rsi_overbought', 70)
            rsi_oversold = params.get('rsi_oversold', 30)
            # Long: at least one of prev_rsi or curr_rsi must be below oversold
            long_mask = va['directions'] == 1
            long_pullback_fail = long_mask & (va['prev_rsi'] >= rsi_oversold) & (va['rsi_values'] >= rsi_oversold)
            # Short: at least one of prev_rsi or curr_rsi must be above overbought
    ...
    def _get_management_arrays_vectorized(
        ...
        # Partial target: compute SL pips then TP pips, take 50%
