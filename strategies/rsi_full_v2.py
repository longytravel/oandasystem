"""
RSI Hidden Divergence Strategy - VERSION 2

Changes from v1:
- swing_strength: Added soft threshold (allows 1 bar tolerance) to reduce cliff-edge
- sl_swing_buffer: Now scales with ATR for volatility adaptation
- Added ATR normalization for more stable parameter behavior

Stability fixes applied:
- swing_strength: Was 2% stable, target 50%+ (soft threshold fix)
- sl_swing_buffer: Was 32% stable, target 60%+ (ATR scaling fix)
- rsi_period: Keep as-is but document the sensitivity

Created: 2026-02-04
Based on pipeline run: GBP_USD_H1_20260204_180055
"""
import numpy as np
import pandas as pd
...
class RSIDivergenceFullFastV2(FastStrategy):
    """
    RSI Hidden Divergence - VERSION 2 with stability fixes.

    Key changes from V1:
    1. Soft threshold swing detection (reduces swing_strength cliff-edge)
    2. ATR-scaled SL buffer (adapts to volatility)
    ...
    def _get_sl_pips(self, signal: FastSignal, params: Dict[str, Any]) -> float:
        ...
        V2 FIX: For swing mode, scale buffer by ATR to adapt to volatility.
        """
        ...
        if sl_mode == 'fixed':
            ...
            # V2 FIX: Scale buffer by ATR (volatility-adaptive)
            # Base buffer from params, scaled by current ATR vs historical average
            base_buffer = params.get('sl_swing_buffer', 10)
            atr_pips = attr.get('atr_pips', 35.0)
            avg_atr_pips = attr.get('avg_atr_pips', 35.0)  # Historical average

            # Scale factor: if current ATR is high, increase buffer; if low, decrease
            # Clamp between 0.5x and 1.5x to avoid extreme values
            if avg_atr_pips > 0:
                atr_scale = min(1.5, max(0.5, atr_pips / avg_atr_pips))
            else:
                atr_scale = 1.0

            scaled_buffer = base_buffer * atr_scale

            return max(swing_dist + scaled_buffer, 10)  # Minimum 10 pips
    ...
    def _find_swings(self, highs: np.ndarray, lows: np.ndarray, strength: int) -> Tuple[List, List]:
        ...
        V2 FIX: Added soft threshold - allows up to 1 bar to break pattern.
        This reduces the cliff-edge sensitivity where swing_strength=5 works
        but 4 and 6 fail catastrophically.

        The idea: A real swing doesn't require EVERY neighbor to be lower/higher.
        Allow some tolerance (1 bar can break the pattern) to make detection
        ...
        # V2 FIX: Allow up to max_breaks neighbors to violate the pattern
        # For strength 3-5: allow 1 break
        # For strength 7-10: allow 2 breaks
        max_breaks = max(1, strength // 4)

        for i in range(strength, n - strength):
            # Swing high - with soft threshold
            is_high = True
            breaks = 0
    ...
    def precompute(self, df: pd.DataFrame) -> List[FastSignal]:
        ...
        # V2 FIX: Calculate average ATR for scaling
        # Use rolling 100-bar average as the "normal" ATR level
        avg_atr = np.zeros_like(atr)
        for i in range(100, len(atr)):
            avg_atr[i] = np.mean(atr[max(0, i-100):i])
        # Fill early bars with the first valid average
        if len(atr) > 100:
            avg_atr[:100] = avg_atr[100]
                        ...
                        if signal_bar < len(closes):
                            ...
                            signals.append(FastSignal(
                                ...
                                attributes={
                                    ...
                                    'avg_atr_pips': avg_atr[signal_bar] / self._pip_size,  # V2: For scaling
        ...
        for s in signals:
            ...
            if params['use_trend_filter']:
                ...
                if ma_key in attr:
                    ma_val = attr[ma_key]
                    if ma_val > 0:
                        if s.direction == 1 and s.price < ma_val:
                            continue
                        if s.direction == -1 and s.price > ma_val:
                            continue
            ...
            if params['use_time_filter']:
                if s.day == 0 and not params['trade_monday']:
                    continue
                if s.day == 4 and not params['trade_friday']:
                    continue
    ...
    def compute_sl_tp(
        ...
        V2 FIX: swing mode now uses ATR-scaled buffer.
        """
        attr = signal.attributes
        ...
        if sl_mode == 'fixed':
            ...
            # V2 FIX: Scale buffer by ATR
            base_buffer = params['sl_swing_buffer']
            atr_pips = attr.get('atr_pips', 35.0)
            avg_atr_pips = attr.get('avg_atr_pips', 35.0)
            ...
            scaled_buffer = base_buffer * atr_scale
            sl_pips = swing_dist + scaled_buffer
        else:
            sl_pips = params['sl_fixed_pips']
